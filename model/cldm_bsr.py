...
class Reflow_ControlLDM(LatentDiffusion):
    ...
    def apply_condition_encoder(self, control):
        # use first_stage_model encode instead of cond_encoder
        posterior = self.first_stage_model.encode(control * 2 - 1)
        if hasattr(posterior, 'latent_dist'):
            c_latent = posterior.latent_dist.mode()
        else:
            c_latent = posterior.mode()
        c_latent = c_latent * self.scale_factor
        return c_latent
    ...
    def configure_optimizers(self):
        ...
        unet_params = list(self.model.diffusion_model.parameters())

        opt = torch.optim.AdamW(itertools.chain(unet_params, params), lr=lr)
